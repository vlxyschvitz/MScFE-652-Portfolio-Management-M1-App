...
Every widget runs client-side: no backend, no build step. Chart.js and KaTeX
are vendored locally as plain files at the repo root (not loaded from a CDN),
so the page works offline and isn't affected by networks that block
...
style.css            Design system — neon/glass dark theme by default, light mode toggle
bg-motion.js         Animated particle-network background (canvas)
utils.js             Shared statistics, matrix algebra, and distribution math
charts-theme.js      Chart.js styling + neon glow plugin, follows the CSS theme
...
chart.umd.js         Vendored Chart.js (no CDN dependency)
katex.min.js         Vendored KaTeX
katex.min.css        Vendored KaTeX stylesheet
KaTeX_*.woff/.woff2/.ttf   Vendored KaTeX math fonts, referenced by katex.min.css
```

Every file sits flat at the repository root — no subfolders at all — so it
deploys cleanly from GitHub Pages' root option (branch `main`, folder
`/ (root)`) with no path changes.
